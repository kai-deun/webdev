import { pat_funcs, pres_funcs, prescriptionUtils, med_funcs, disp_funcs } from "./singletons.js";
...
export class EventBinder {
    ...
    bindStaticContent() {
        ...
        const newPrescriptionBtn = document.querySelector('.header-actions .btn-primary');
        if (newPrescriptionBtn && newPrescriptionBtn.textContent.includes('New Prescription')) {
        ...
        document.querySelectorAll('.search-box').forEach(box => {
            box.addEventListener('input', (e) => prescriptionUtils.handleSearch(e.target));
        });

        //filter selector binder
        document.querySelectorAll('.filter-select').forEach(select => {
            select.addEventListener('change', (e) => prescriptionUtils.handleFilter(e.target));
        });

        //tab buttons binder
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.addEventListener('click', (e) => prescriptionUtils.switchTab(e.target));
        });
