...
export class PrescriptionUtilities {
  ...
  async loadPatients() {
    try {
      ...
      if (patientsList) {
        patientsList.innerHTML =
          "<p>Error loading patients. Please check the console for details.</p>";
      }
    }
  }
  ...
  async loadPrescriptions() {
    try {
      ...
      if (prescriptionsList) {
        prescriptionsList.innerHTML =
          "<p>Error loading prescriptions. Please check the console for details.</p>";
      }
    }
  }
  ...
  clearPrescriptionForm() {
    ...
    document.getElementById("prescription-date").value = new Date()
      .toISOString()
      .split("T")[0];
  ...
  activateEditMode(prescriptionId) {
    ...
    try {
      const data = this.fetchPhpFunction(
        `getPrescriptionDetails&id=${prescriptionId}`
      );
      ...
      if (data.success) {
        ...
        if (prescriptionForm) {
          ...
          document.querySelector(".content-section h3").innerHTML =
            "Edit Prescription";
          ...
          patient_select_replacement.setAttribute(
            "name",
            orig_patient_select.name
          );
          patient_select_replacement.setAttribute(
            "value",
            prescription_details.patient_name
          );
          ...
          //diagnosis & notes
          document.getElementById("diagnosis").value =
            prescription_details.diagnosis;
          ...
          if (oldBtn) {
            ...
            newButton.addEventListener("click", (e) => {
              ...
              const getCurrentPrescription =
                prescriptionObject.getCurrentPrescription();
              ...
              document
                .getElementById("prescriptions-tab")
  ...
  searchPatients(searchTerm) {
    const filteredPatients = prescriptionObject
      .getPatients()
      .filter((patient) => {
        const fullName =
  ...
  searchPrescriptions(searchTerm) {
    const filteredPrescriptions = prescriptionObject
      .getPrescriptions()
      .filter((prescription) => {
        const patient = prescriptionObject
          .getPatients()
          .find((p) => p.patient_id === prescription.patient_id);
